...
def make_question():
    ...
    op = random.randint(1, 4) # 1-4 사이의 임의 수를 op에 저
    ...
    if op == 2:   # 연산자 추가 / op값이 2면 뺄셈 문제 생성
        q = q+ "-"
    if op == 3:   # 연산자 추가 / op값이 3이면 곱셈 문제 생성
        q = q+ "*"
    if op == 4:
        q = q+ "/"   
...
for x in range(5):  # 5문제 생성
    ...
    print(q)  # 문제 출력
    ans = input("=")    # 사용자 정답 입력
    r = int(ans)  # 정답 타입을 정수로 변환

    if eval(q) == r:   #문제 q를 컴퓨터가 계산한 결과인 eval(q)와 비교하여 정답 및 오답 판단
        print("정답!")
        sc1 = sc1 + 1
    else:
        print("오답!")
